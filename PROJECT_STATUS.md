...
- `FormulaEvidence` 新增可空的 `explanation_text`。解释只能是公式行内明确存在的自然语言原文；
  纯公式、尾随状态图标签、未闭合表达和多关系证明块不会被自动解释。
- 学习页和复核页同时展示“课件原文解释”或明确的缺失状态。7 份真实课件共保留 32 条高置信
  公式，其中 10 条带可抽取解释；30 章复核样本含 28 条公式，其中 10 条带解释。
