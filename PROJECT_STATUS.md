...
- 闪卡采用高置信定义问答 baseline：仅从“术语：定义”、系词定义或自然语言等号定义中生成，
  答案、完整证据句和页码引用同时保存，不再随机从长段原文中挖空词语。
...
- 每章新增重点概念列表：只复用通过定义型闪卡规则的原文术语，并同时展示完整证据句和页码。
- 公式证据要求关系符与函数、求和、复杂度、幂或范数等数学结构同时存在；自然语言等号定义
  仍可进入概念卡，但不会污染公式类别。
- 人工通读首批 30 章后发现网址/小数被句号切断、版面列文本被拼成伪术语、随机挖空废卡和
  自然语言等号误判公式等系统性失败。修正后学习材料优先使用完整 page 文本，过滤网址、样板
  文本、稀疏视觉图注和版面标签，并将上述失败样本固化为回归测试。
- 摘要、概念和闪卡候选额外拒绝连续点号及 Unicode 省略号，防止视觉抽取或原页截断内容进入
  学习材料；公式证据保持独立处理，以保留数学表达中合法的省略记号。
- 完整课件项目符号若缺少终止标点，只在摘要展示文本末尾补一个句号；已有单句号原样保留。
  行内项目符号会先拆开，并过滤悬空助动词、未闭合括号、断词和部分悬空进行时结构。
- 对 7 份公开课件重新完成整门课程构建：41 个可用章节、160 条摘要、25 个重点概念、25 张
  高置信定义型闪卡、32 条公式证据和 110 个去重引用页；产物位于
  `artifacts/public_course_study_ui/index.html`。
...
- 首批 30 章已完成人工通读并暴露生成链路缺陷；修正规则后从 41 个可用章节重新抽取 30 章，
  覆盖全部 7 份课件、118 条摘要、22 个概念/闪卡、28 条公式和 84 个去重证据页。内容指纹已
  变化，因此旧复核决定不会被错误继承；新包位于
  `artifacts/study_material_review/index.html`，需要针对修正版重新复核。

### 3.33 人工复核通过与离线公式解释

- 用户完成修正版 30 章复核，确认未发现明显问题；该版本作为后续生成改动的人工回归基线。
- `FormulaEvidence` 新增可空的 `explanation_text`。说明必须来自独立于公式行的相邻课件文本；
  等号右侧即使是自然语言也不会再复制成“解释”。纯公式、尾随状态图标签、未闭合表达和多关系
  证明块不会被自动解释。
- 学习页和复核页同时展示“课件相邻说明”或“未找到独立说明”。7 份真实课件共保留 32 条
  高置信公式，其中 10 条带独立相邻说明，公式原文与说明重复计数为 0。
- 修复状态图单字母标签被当作小写续行拼入公式的问题：独立一行的 `a`、`s` 等不再与前一行
  合并。真实课件中的 `π*(s)` 已恢复为 `π*(s) = optimal action from state s`，全量公式尾随
  连续单字母标签扫描为 0。

### 3.34 高置信离线基础题

- `ChapterStudyGuide` 新增 `questions`，CLI 的三个学习材料命令新增 `--questions` 每章数量上限；
  构建报告单独记录题目数。
- 基础题只复用已通过定义质量门槛的术语与答案，题型固定为短答，答案必须逐字存在于证据文本，
  并保留独立页码引用；题面使用 `What does “术语” mean?`，避免单复数语法问题。
- 学习页新增可展开答案的“基础题”标签页，复核页新增独立题目类别和导出决定。真实 7 份课件
...
- 已实现 BM25/Dense 离线 extractive 基线、多证据答案、章节摘要、定义型问答闪卡、生成评测和
