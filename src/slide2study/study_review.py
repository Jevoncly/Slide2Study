...
function materialBlock(field,label){const wrap=document.createElement('section');wrap.className='material';const h=document.createElement('h3');h.textContent=`${label} · ${current()[field].length}`;const list=document.createElement('div');list.className='items';current()[field].forEach(x=>{const row=document.createElement('div');row.className='item';const text=document.createElement('div');if(field==='summary')text.textContent=x.text;else if(field==='concepts'){const t=document.createElement('span');t.className='term';t.textContent=x.term+' — ';text.append(t,document.createTextNode(x.evidence_text))}else if(field==='flashcards')text.textContent=`${x.front}  →  ${x.back}`;else{text.textContent=x.formula_text;const e=document.createElement('div');e.className='muted';e.textContent=x.explanation_text?`课件原文解释：${x.explanation_text}`:'本页未提供可可靠抽取的文字解释。';text.append(e)}row.append(text,cite(x.citation));list.append(row)});if(!current()[field].length){const empty=document.createElement('div');empty.className='muted';empty.textContent='无可复核内容';list.append(empty)}wrap.append(h,list);return wrap}
