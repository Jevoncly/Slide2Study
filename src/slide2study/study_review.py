...
def build_study_review_pack(
    ...
    for index, guide in enumerate(guides, 1):
        ...
        citations = [
            item["citation"]
            for field in ("summary", "concepts", "flashcards", "formulas", "questions")
...
const app=JSON.parse(document.getElementById('review-data').textContent),key='slide2study-review:__FINGERPRINT__';let index=0,pageIndex=0;let reviews={};try{reviews=JSON.parse(localStorage.getItem(key)||'{}')}catch{reviews={}}const categories=[['summary','摘要'],['concepts','概念'],['flashcards','闪卡'],['formulas','公式/参数'],['questions','基础题']];
...
function materialBlock(field,label){const wrap=document.createElement('section');wrap.className='material';const h=document.createElement('h3');h.textContent=`${label} · ${current()[field].length}`;const list=document.createElement('div');list.className='items';current()[field].forEach(x=>{const row=document.createElement('div');row.className='item';const text=document.createElement('div');if(field==='summary')text.textContent=x.text;else if(field==='concepts'){const t=document.createElement('span');t.className='term';t.textContent=x.term+' — ';text.append(t,document.createTextNode(x.evidence_text))}else if(field==='flashcards')text.textContent=`${x.front}  →  ${x.back}`;else if(field==='questions')text.textContent=`${x.prompt}  →  ${x.answer}`;else{text.textContent=x.formula_text;const e=document.createElement('div');e.className='muted';e.textContent=x.explanation_text?`课件相邻说明：${x.explanation_text}`:'未找到独立于公式原文的可靠说明。';text.append(e)}row.append(text,cite(x.citation));list.append(row)});if(!current()[field].length){const empty=document.createElement('div');empty.className='muted';empty.textContent='无可复核内容';list.append(empty)}wrap.append(h,list);return wrap}
